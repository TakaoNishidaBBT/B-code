...
*/
	...
	bframe.tree = function(target) {
		...
		function setEditorOverlay() {
			...
			editor_overlay = document.getElementById('bframe_tree_editor_overlay');
			if(!editor_overlay) {
				editor_overlay = document.createElement('div');
				editor_overlay.id = 'bframe_tree_overlay_div';
				editor_overlay.style.backgroundColor = '#f00';
				editor_overlay.style.opacity = 0;

				editor_overlay.style.position = 'absolute';
				editor_overlay.style.width = '100%';
				editor_overlay.style.height = '100%';
				editor_overlay.style.top = 0;
				editor_overlay.style.left = 0;
				...
				editor_container.appendChild(editor_overlay);
			}
		}
		...
		function selectFolder(node_id) {
			...
			if(current_node.id() == node_id) return;
			currentObject(node_id);

			var param = 'terminal_id='+terminal_id+'&node_id='+encodeURIComponent(node_id.substr(1));

			httpObj = createXMLHttpRequest(selectFolderResponse);
			eventHandler(httpObj, property.module, property.file, property.method.selectFolder, 'POST', param);
		...
		function openEditor(node_id, mode='temporary') {
		...
		function tabControl() {
			...
			function getTabIndex(obj) {
				for(var i=0; i < tabs.length; i++) {
					if(tabs[i].obj == obj) return i;
				}
			}

			function getIndex(element) {
				for(var i=0; i < tabs.length; i++) {
					if(tabs[i].obj.element() == element) return i;
				}
			}
...
console.log('f_name', f_name);
				...
				this.setZindex = function(number) {
					z = number;
					if(!visible) li.style.zIndex = number;
				}
		...
		function dragControl(w) {
			...
			this.dragStart = function(event, node_id) {
				...
				if(editor_overlay) {
					editor_overlay.style.display = 'block';
				}
			...
			function onMouseUp(event) {
				...
				if(editor_overlay && !context_menu.opened()) {
					editor_overlay.style.display = 'none';
				}
				if(clone_node.style.visibility == 'visible') {
					clone_node.style.top = 0;
